...
*
*   Why is this needed?
*   - In SMC some blocks are superblocks, which output much higher amounts of coins
...
{
    bool isBlockRewardValueMet = false;
    strErrorRet = "";
    ...
    if(fDebug) LogPrintf("block.vtx[0].GetValueOut() %lld !=0 blockReward %lld\n", block.vtx[0].GetValueOut(), blockReward);
...
{
    if(fLiteMode) return; // disable all SMC specific functionality
...
bool CMasternodePayments::IsScheduled(CMasternode& mn, int nNotBlockHeight)
{
    ...
    {
        LogPrint("CMasternodePayments::IsScheduled","!CMasternodePayments::IsScheduled");
        return false;
    }
    ...
    LogPrint("CMasternodePayments::IsScheduled",",nCachedBlockHeight:%d nNotBlockHeight:%d mnpayee:%s\n",nCachedBlockHeight,nNotBlockHeight,HexStr(mnpayee));
    CScript payee;
...
bool CMasternodeBlockPayees::GetBestPayee(CScript& payeeRet)
{
    ...
    int nVotes = -1;
    BOOST_FOREACH(CMasternodePayee& payee, vecPayees) {
        LogPrint("mnpayments", "CMasternodePayee getvotecount:%d nVotes:%d %s\n",payee.GetVoteCount(),nVotes,HexStr(payee.GetPayee()));
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
    ...
    LogPrint("mnpayments","CMasternodeBlockPayees::IsTransactionValid -- ERROR: Missing required payment, possible payees: '%s', amount: %f SMC\n", strPayeesPossible, (float)nMasternodePayment/COIN);
    return false;
}
