        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if(nCoinType == ONLY_DENOMINATED) {
                    ...
                    found = pcoin->vout[i].nValue == 500000*COIN;//tbd
    ...
    for (auto& outpoint : setWalletUTXO) {
        ...
        for (unsigned int i = 0; i < wtx.vout.size(); i++) {
            ...
            if(fAnonymizable) {
                ...
                if(fMasterNode && wtx.vout[i].nValue == 500000*COIN) continue;//tbd
...
bool CWallet::SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vecTxInRet, CAmount& nValueRet, int nPrivateSendRoundsMin, int nPrivateSendRoundsMax) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        if(fMasterNode && out.tx->vout[out.i].nValue == 500000*COIN) continue; //masternode input  tbd
                                ...
                                int& nChangePosRet, std::string& strFailReason, const CCoinControl* coinControl, bool sign, AvailableCoinsType nCoinType, bool fUseInstantSend)
{
    ...
    CAmount nValue = 0;
    unsigned int nSusmcactFeeFromAmount = 0;
    BOOST_FOREACH (const CRecipient& recipient, vecSend)
    {
        ...
        if (recipient.fSusmcactFeeFromAmount)
            nSusmcactFeeFromAmount++;
    }
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                BOOST_FOREACH (const CRecipient& recipient, vecSend)
                {
                    ...
                    if (recipient.fSusmcactFeeFromAmount)
                    {
                        txout.nValue -= nFeeRet / nSusmcactFeeFromAmount; // Susmcact fee equally from each selected recipient

                        if (fFirst) // first receiver pays the remainder not divisible by output count
                        {
                            fFirst = false;
                            txout.nValue -= nFeeRet % nSusmcactFeeFromAmount;
                        }
                    }

                    if (txout.IsDust(::minRelayTxFee))
                    {
                        if (recipient.fSusmcactFeeFromAmount && nFeeRet > 0)
                        {
                            if (txout.nValue < 0)
                ...
                if (fUseInstantSend && nValueIn > sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE)*COIN) {
                    strFailReason += " " + strprintf(_("InstantSend doesn't support sending values that high yet. Transactions are currently limited to %1 SMC."), sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE));
                    return false;
                }
                ...
                if (nChange > 0)
                {
                    ...
                    if (nCoinType == ONLY_DENOMINATED) {
                        ...
                        if (nSusmcactFeeFromAmount > 0 && newTxOut.IsDust(::minRelayTxFee))
                        {
                            CAmount nDust = newTxOut.GetDustThreshold(::minRelayTxFee) - newTxOut.nValue;
                            newTxOut.nValue += nDust; // raise change until no more dust
                            for (unsigned int i = 0; i < vecSend.size(); i++) // susmcact from first recipient
                            {
                                if (vecSend[i].fSusmcactFeeFromAmount)
                                {
