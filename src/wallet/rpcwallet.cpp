...
// Copyright (c) 2014-2017 The SMC Core developers
...
UniValue getaccountaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nDEPRECATED. Returns the current SMC address for receiving payments to this account.\n"
            "\nArguments:\n"
...
static void SendMoney(const CTxDestination &address, CAmount nValue, bool fSusmcactFeeFromAmount, CWalletTx& wtxNew, bool fUseInstantSend=false, bool fUsePrivateSend=false)
{
    ...
    int nChangePosRet = -1;
    CRecipient recipient = {scriptPubKey, nValue, fSusmcactFeeFromAmount};
    vecSend.push_back(recipient);
    if (!pwalletMain->CreateTransaction(vecSend, wtxNew, reservekey, nFeeRequired, nChangePosRet,
                                         strError, NULL, true, fUsePrivateSend ? ONLY_DENOMINATED : ALL_COINS, fUseInstantSend)) {
        if (!fSusmcactFeeFromAmount && nValue + nFeeRequired > pwalletMain->GetBalance())
...
UniValue sendtoaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 7)
        throw runtime_error(
            "sendtoaddress \"smcaddress\" amount ( \"comment\" \"comment-to\" susmcactfeefromamount use_is use_ps )\n"
            ...
            "5. susmcactfeefromamount  (boolean, optional, default=false) The fee will be deducted from the amount being sent.\n"
            "                             The recipient will receive less amount of SMC than you enter in the amount field.\n"
    ...
    bool fSusmcactFeeFromAmount = false;
    if (params.size() > 4)
        fSusmcactFeeFromAmount = params[4].get_bool();
    ...
    SendMoney(address.Get(), nAmount, fSusmcactFeeFromAmount, wtx, fUseInstantSend, fUsePrivateSend);
...
UniValue instantsendtoaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            "instantsendtoaddress \"smcaddress\" amount ( \"comment\" \"comment-to\" susmcactfeefromamount )\n"
    ...
    SendMoney(address.Get(), nAmount, fSusmcactFeeFromAmount, wtx, true);
...
UniValue getreceivedbyaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "getreceivedbyaddress \"smcaddress\" ( minconf addlockconf )\n"
            "\nReturns the total amount received by the given smcaddress in transactions with specified minimum number of confirmations.\n"
            "\nArguments:\n"
...
UniValue sendfrom(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 3 || params.size() > 7)
        throw runtime_error(
            "sendfrom \"fromaccount\" \"tosmcaddress\" amount ( minconf addlockconf \"comment\" \"comment-to\" )\n"
            "\nDEPRECATED (use sendtoaddress). Sent an amount from an account to a smc address."
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 8)
        throw runtime_error(
            "sendmany \"fromaccount\" {\"address\":amount,...} ( minconf addlockconf \"comment\" [\"address\",...] susmcactfeefromamount use_is use_ps )\n"
            ...
            "    {\n"
            "      \"address\":amount     (numeric or string) The smc address is the key, the numeric amount (can be string) in " + CURRENCY_UNIT + " is the value\n"
            "      ,...\n"
            "    }\n"
            ...
            "6. susmcactfeefromamount   (string, optional) A json array with addresses.\n"
            "                           The fee will be equally deducted from the amount of each selected address.\n"
            "                           Those recipients will receive less smcs than you enter in their corresponding amount field.\n"
            "                           If no addresses are specified here, the sender pays the fee.\n"
            "    [\n"
            "      \"address\"            (string) Susmcact fee from this address\n"
            "      ,...\n"
            "    ]\n"
    ...
    UniValue susmcactFeeFromAmount(UniValue::VARR);
    if (params.size() > 5)
        susmcactFeeFromAmount = params[5].get_array();
    ...
    BOOST_FOREACH(const string& name_, keys)
    {
        ...
        bool fSusmcactFeeFromAmount = false;
        for (unsigned int idx = 0; idx < susmcactFeeFromAmount.size(); idx++) {
            const UniValue& addr = susmcactFeeFromAmount[idx];
            if (addr.get_str() == name_)
                fSusmcactFeeFromAmount = true;
        }

        CRecipient recipient = {scriptPubKey, nAmount, fSusmcactFeeFromAmount};
        vecSend.push_back(recipient);
    }
...
UniValue encryptwallet(const UniValue& params, bool fHelp)
{
    ...
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending smc\n"
    ...
    StartShutdown();
    return "Wallet encrypted; SMC Core server stopping, restart to run with encrypted wallet. The keypool has been flushed and a new HD seed was generated (if you are using HD). You need to make a new backup.";
}
