...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "SMCNetwork") type = SMCNetwork;
    if(strType == "SMCNetworkVariable") type = SMCNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == SMCNetwork) s = "SMCNetwork";
    if(type == SMCNetworkVariable) s = "SMCNetworkVariable";
