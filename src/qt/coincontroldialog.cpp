...
bool CoinControlDialog::fSusmcactFeeFromAmount = false;
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    if (nQuantity > 0)
    {
        ...
        // in the susmcact fee from amount case, we can tell if zero change already and susmcact the bytes, so that fee calculation afterwards is accurate
        if (CoinControlDialog::fSusmcactFeeFromAmount)
        ...
        if (nPayAmount > 0)
        {
            ...
            if (nChange > 0 && nChange < MIN_CHANGE)
            {
                ...
                if (txout.IsDust(::minRelayTxFee))
                {
                    if (CoinControlDialog::fSusmcactFeeFromAmount) // dust-change will be raised until no dust
            ...
            if (nChange == 0 && !CoinControlDialog::fSusmcactFeeFromAmount)
                nBytes -= 34;
        }
