...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
    ...
    Q_FOREACH(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        {   // PaymentRequest...
            ...
            for (int i = 0; i < details.outputs_size(); i++)
            {
                ...
                CRecipient recipient = {scriptPubKey, nAmount, rcp.fSusmcactFeeFromAmount};
        ...
        {   // User-entered smc address / amount:
            ...
            CRecipient recipient = {scriptPubKey, rcp.amount, rcp.fSusmcactFeeFromAmount};
            vecSend.push_back(recipient);
...

    {
        ...
        if(recipients[0].fUseInstantSend && total > sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE)*COIN){
            Q_EMIT message(tr("Send Coins"), tr("InstantSend doesn't support sending values that high yet. Transactions are currently limited to %1 SMC.").arg(sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE)),
        ...
        if (fSusmcactFeeFromAmount && fCreated)
        ...
        if(!fCreated)
        {
            if(!fSusmcactFeeFromAmount && (total + nFeeRequired) > nBalance)
            {
