...
SendCoinsEntry *SendCoinsDialog::addEntry()
{
    ...
    connect(entry, SIGNAL(susmcactFeeFromAmountChanged()), this, SLOT(coinControlUpdateLabels()));
...
void SendCoinsDialog::coinControlUpdateLabels()
{
    ...
    CoinControlDialog::fSusmcactFeeFromAmount = false;
    for(int i = 0; i < ui->entries->count(); ++i)
    {
        SendCoinsEntry *entry = qobject_cast<SendCoinsEntry*>(ui->entries->itemAt(i)->widget());
        if(entry && !entry->isHidden())
        {
            ...
            if (rcp.fSusmcactFeeFromAmount)
                CoinControlDialog::fSusmcactFeeFromAmount = true;
        }
    }
