...
CBlockTemplate* CreateNewBlock(const CChainParams& chainparams, const CScript& scriptPubKeyIn)
{
...

    {
        ...
        LogPrintf("CreateNewBlock(): total size %u txs: %u fees: %ld sigops %d height:%d\n", nBlockSize, nBlockTx, nFees, nBlockSigOps,nHeight);
...
// ***TODO*** ScanHash is not yet used in SMC
//
...
void static BitcoinMiner(const CChainParams& chainparams, CConnman& connman)
{
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("SMCMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            ...
            LogPrintf("SMCMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("SMCMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("SMCMiner -- runtime error: %s\n", e.what());
        return;
    }
}
